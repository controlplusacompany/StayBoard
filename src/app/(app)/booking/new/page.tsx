...
import { getStoredRooms, addBooking, getBookingsForRoom, getAvailableRoomCount, getRoomBlockedDates, getSelectedProperty } from '@/lib/store';
...
function BookingFlow() {
  ...
  }, [selectedRoom, propertyRooms, formData.pricePerNight]);
  ...
  const handleSubmit = async () => {
    if (!formData.guestName || !formData.guestPhone) {
      toast("Guest name and phone are required", "error");
      return;
    }
    ...
    if (bookingType === 'walk-in') {
      ...
      if (formData.nationality === 'indian' && !formData.idNumber) {
        toast("ID Number is mandatory for check-in", "error");
        return;
      }
      if (formData.nationality === 'international' && !formData.passportNumber) {
        toast("Passport details are mandatory for international guests", "error");
        return;
      }
    }
  ...
  return (
    <div className="max-w-6xl mx-auto w-full flex flex-col min-h-screen pt-4 pb-20 md:pb-8 px-6">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 pb-6 border-b border-border-subtle gap-4">
        ...
        <div className="flex gap-1 p-1 bg-bg-sunken rounded-xl border border-border-subtle self-start md:self-center">
          <button 
            onClick={() => setBookingType('walk-in')}
            className={`px-6 py-2 rounded-lg text-xs font-bold transition-all uppercase tracking-wider ${bookingType === 'walk-in' ? 'bg-accent text-white shadow-md' : 'text-ink-muted hover:text-ink-secondary'}`}
          >
            Walk-in Now
          </button>
          <button 
            onClick={() => setBookingType('reservation')}
            className={`px-6 py-2 rounded-lg text-xs font-bold transition-all uppercase tracking-wider ${bookingType === 'reservation' ? 'bg-accent text-white shadow-md' : 'text-ink-muted hover:text-ink-secondary'}`}
          >
            Future Reservation
          </button>
        </div>
          ...
          <section className="flex flex-col gap-6">
              ...
              <div className="field">
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">
                  {propertyRooms.map(r => (
                    <button
                      key={r.id}
                      onClick={() => updateField('selectedRoomId', r.id)}
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="flex flex-col gap-1 px-3 py-2 bg-bg-sunken rounded-xl border border-border-subtle">
                <label className="text-[10px] font-bold text-ink-muted uppercase">Adults</label>
              ...
              <div className="flex flex-col gap-1 px-3 py-2 bg-bg-sunken rounded-xl border border-border-subtle">
                <label className="text-[10px] font-bold text-ink-muted uppercase">Children</label>
                <div className="flex items-center justify-between">
              ...
              <div className="flex flex-col gap-1 px-3 py-2 bg-bg-sunken rounded-xl border border-border-subtle">
                <label className="text-[10px] font-bold text-ink-muted uppercase">Infants</label>
                <div className="flex items-center justify-between">
              ...
              <div className="field">
                ...
                <div className="relative">
                   ...
                   <input
                     ...
                     value={formData.guestPhone.replace('+91 ', '')}
              ...
              <div className="field">
                ...
                <div className="flex gap-1 p-1 bg-bg-sunken rounded-lg border border-border-subtle h-[46px]">
                   <button 
                     ...
                     className={`flex-1 rounded-md text-[10px] font-bold uppercase transition-all ${formData.nationality === 'indian' ? 'bg-white shadow text-ink-primary' : 'text-ink-muted'}`}
                   >🇮🇳 Indian</button>
                   <button 
                     onClick={() => updateField('nationality', 'international')}
                     className={`flex-1 rounded-md text-[10px] font-bold uppercase transition-all ${formData.nationality === 'international' ? 'bg-white shadow text-ink-primary' : 'text-ink-muted'}`}
                   >🌎 Intl</button>
                </div>
              </div>
        ...
        <div className="lg:col-span-4 flex flex-col">
          <div className="bg-white rounded-2xl border border-border-strong p-6 sticky top-24 shadow-xl flex flex-col gap-6">
            <div className="flex flex-col gap-1">
               <span className="text-[10px] font-bold text-accent uppercase tracking-widest">{bookingType === 'walk-in' ? 'Check-in Summary' : 'Reservation Summary'}</span>
            ...
            <div className="flex flex-col gap-3 py-4 border-y border-border-subtle">
               ...
               <div className="flex justify-between pt-2 mt-1 border-t border-dashed text-sm">
                 ...
                 <span className="font-mono font-bold text-danger text-base">₹{balanceDue.toLocaleString()}</span>
               </div>
            </div>

            <button
               onClick={handleSubmit}
               className="btn btn-accent w-full py-4 rounded-xl flex items-center justify-center gap-2 font-bold shadow-lg shadow-accent/20"
            >
